# Audit report — Stage 25

**Date**: 2026-07-03T09:44:48Z
...
```
Audit report (2026-07-03T09:44:31Z)
=============================================
  mock_predictions                  0
...
compliance/decision-logs/2026-07-02_stage25_post_ga_ops.md
compliance/decision-logs/2026-07-02_strategic_audit_and_post_ga_roadmap.md
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — Stage 25 is post-GA OPERATIONS: additive real code
  only (Art-72 anomaly sweep + signed audit rows, pgaudit enablement, PQC rotation drill, shard-router exactly-once
  guards, ops cascade endpoints reading ONLY the signed audit_chain, nightly OpenSSL-3.5 crypto gate, DVC skill,
  Langfuse-overlay live fixes). No `random.*`/mock/`RESPONSES={}`/`MODELS=[]` introduced; no de-mock surface touched
  (the legacy de-mock is Stage 28 per ADR `2026-07-02_strategic_audit_and_post_ga_roadmap.md`). Justified in
  `knowledge-base/KB_TASK_LOG.md` (Stage 25 entry); ADR `2026-07-02_stage25_post_ga_ops.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE.** Stage 25 added NO
  actuator emitter. The heuristic flags (a) `backend/.pytest_cache/v/cache/nodeids` — a pytest CACHE file (test-id
  strings), not code; (b) `backend/agents/base_agent.py` — a FIRST-COMMIT legacy-demo-path file (G-082 surface),
  untouched by this stage. The `shard_router` runs incidents through the SAME runtime `execute` node that routes via
  `safety.validator` (Stage 17); the only real actuator emitter remains `master.dispatch_order`, validator-gated +
  trace-paired. Verified: full suite 365 passed / 10 skipped / 0 failed on the live stack this session.
