# Audit report — Stage 34

**Date**: 2026-07-17T16:14:24Z
**Task doc**: tasks/STAGE_34_TBD.md
**Baseline**: 3
**Audit total**: 3
...
```
Audit report (2026-07-17T16:14:09Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 3. **WAIVED via `--no-baseline-drop`** — Stage 34 is a FRONTEND honesty cleanup. It REMOVED
  fabrications (`getMockModelMetrics`/`getMockEmbodiedComparison` + the model-metrics hardcoded fake array + the
  simulation fabricated init), but those were audit-INVISIBLE TS object literals (not `Math.random`/mock patterns), so
  `audit.sh` cannot see the improvement — it stays 3 (the documented `_generate_heuristic_actions` G-052 false-positive).
  Grep-verified: `Math.random`/`getMock` in `frontend-nextjs/src` = 0 (the honestly-labelled `detRand` demo layout
  excepted). Net honesty gain. Justified in `KB_TASK_LOG.md`; ADR `2026-07-13_stage34_frontend_realdata_honesty.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE + not-applicable** — the
  coarse heuristic matches "actuator" across pre-existing `.pyc`/venv/legacy backend files; Stage 34 touched ONLY
  `frontend-nextjs/` (no backend, no actuator path). The sole actuator emitter remains `master.dispatch_order`
  (validator + trace-paired). `next build` type-checks strictly (exit 0); backend audit + chain unaffected.
