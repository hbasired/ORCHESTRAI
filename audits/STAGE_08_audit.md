# Audit report — Stage 08

**Date**: 2026-06-13T13:01:13Z
...
```
Audit report (2026-06-13T13:01:09Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 396. **Justified** in KB_TASK_LOG (2026-06-13 Stage 8 entry) + ADR `2026-06-13_world_model_causal_diagnose.md` D6: additive ML stage (trained TTF world model + causal attribution v1); it de-mocks `world_model.py` by removing `np.random.randn`, but those are grep-invisible to `audit.sh` (same class as G-047), so the counted baseline holds flat honestly; no in-lane grep-counted theatre exists (remaining hits are Stage-9/10/11 files). Closing with `--no-baseline-drop`.
