# Audit report — Stage 13

**Date**: 2026-06-15T19:08:16Z
**Task doc**: tasks/STAGE_13_cdc_ingestion.md
**Baseline**: 364
**Audit total**: 364
...
```
Audit report (2026-06-15T19:08:12Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — justified in `knowledge-base/KB_TASK_LOG.md` (Stage 13 entry): CDC ingestion is native-SQL (trigger + outbox + `pg_notify`) + a real psycopg listener that injects real `InjectRequest`s — it adds no `random.*`/mock theatre the grep counts, and removes none. New code is additive. ADR `2026-06-15_stage13_cdc_ingestion.md`.
