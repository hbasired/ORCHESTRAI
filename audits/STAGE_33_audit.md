# Audit report — Stage 33

**Date**: 2026-07-13T13:09:31Z
**Task doc**: tasks/STAGE_33_TBD.md
**Baseline**: 3
**Audit total**: 3
...
```
Audit report (2026-07-13T13:09:19Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 3. **WAIVED via `--no-baseline-drop`** — Stage 33 is an ADDITIVE safety/oversight hardening
  increment (capability tokens + behavioural-monitor hook). Zero new `random.*`/mock introduced — the capability token
  uses stdlib `hmac`/`os.urandom` (NOT the theatrical-fallback `random`), and the audit grep confirms 3 (the residual =
  the documented `_generate_heuristic_actions` G-052 false-positive). Justified in `KB_TASK_LOG.md`; ADR
  `2026-07-13_stage33_safety_oversight_hardening.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE — and this stage
  STRENGTHENED the guarantee.** The heuristic matches "actuator" across `.pyc`/venv/legacy files; the actual actuator
  bridge `safety/sil_bridge.py` explicitly imports + calls `safety.validator.validate()` (re-validation path) AND now
  verifies a validator-minted capability token — the G-075 fix makes `sil_bridge` refuse any Decision not produced by
  the validator. The sole live emitter remains `master.dispatch_order` (validator + trace-paired). 33 safety tests
  (incl. 7 capability-token) pass; audit-chain verify exit 0.
