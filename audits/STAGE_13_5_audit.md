# Audit report — Stage 13_5

**Date**: 2026-06-19T14:41:34Z
**Task doc**: tasks/STAGE_13_5_pqc_foundations.md
**Baseline**: 364
**Audit total**: 364
...
```
Audit report (2026-06-19T14:41:31Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — justified in `knowledge-base/KB_TASK_LOG.md` (Stage 13.5 entry): the PQC layer (`backend/crypto/`) is real FIPS-204 ML-DSA-65 + HMAC-SHA-384 — no `random.*`/mock/RSA/ECDSA theatre the grep counts; new code is additive. The classical-crypto gate false-positive (the word "ECDSA" in a comment) was reworded away. ADR `2026-06-15_stage13_5_pqc_foundations.md`.
