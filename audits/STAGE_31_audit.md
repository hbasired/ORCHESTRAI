# Audit report — Stage 31

**Date**: 2026-07-13T05:39:57Z
**Task doc**: tasks/STAGE_31_TBD.md
**Baseline**: 3
**Audit total**: 3
...
```
Audit report (2026-07-13T05:39:40Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 3. **WAIVED via `--no-baseline-drop`** — Stage 31 is an ADDITIVE detector-hardening
  increment (learned injection tier + continuous behavioural monitor). Zero new `random.*`/mock introduced; audit.sh
  re-verified 3. The learned tier REDUCES the detector's real held-out FPR (0.0156→0.0) — a genuine defence gain (the
  audit grep can't see detector quality). Residual 3 = the documented `_generate_heuristic_actions` G-052
  false-positive. Justified in `KB_TASK_LOG.md`; ADR `2026-07-13_stage31_detector_eval_hardening.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE** (same class as
  Stage-24..30): the coarse heuristic matches "actuator" across `.pyc`, `.pytest_cache`, venv third-party, and legacy
  demo agents. Stage 31 added NO actuator path — the injection classifier + behavioural monitor are read-only detectors;
  the sole actuator emitter remains `master.dispatch_order` (validator + trace-paired), and this stage EXPLICITLY
  reaffirms that the binding actuation gate is `safety/validator` (Rule 3), detectors being defence-in-depth. 30
  security+red-team tests pass; audit-chain verify exit 0.
