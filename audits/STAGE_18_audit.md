# Audit report — Stage 18

**Date**: 2026-06-21T12:48:10Z
**Task doc**: tasks/STAGE_18_pqc_wave2.md
**Baseline**: 364
**Audit total**: 364
...
```
Audit report (2026-06-21T12:47:52Z)
=============================================
  mock_predictions                  0
...
compliance/decision-logs/2026-06-21_stage18_pqc_wave2.md
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — Stage 18 adds real PQC code (ML-KEM-768 via kyber-py, SLH-DSA-SHA2-128s via OpenSSL 3.5, hybrid-TLS sidecar, 4-key rotation, SBOM) with NO `random.*`/mock/dict-literal theatre the grep counts; purely additive. The NEW `audit.sh` classical-crypto gate also passes (zero real RSA/EC API calls in backend/crypto+a2a). Justified in `knowledge-base/KB_TASK_LOG.md` (Stage 18 entry). ADR `2026-06-21_stage18_pqc_wave2.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE — Stage 18 added NO actuator code (it is a crypto/TLS stage).** The flagged files are the unchanged pre-existing `agents/*`/`ml/*`/`services/*` + the `safety/*` docstrings (which describe the gate) + `.pyc`/`venv` noise — none dispatch an actuator. The real actuator paths remain validator-gated + trace-paired (Stage 17, CI `safety-contract-tests`); no new unguarded path was introduced.
