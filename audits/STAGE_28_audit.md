# Audit report — Stage 28

**Date**: 2026-07-12T09:49:29Z
**Task doc**: tasks/STAGE_28_graphrag_adoption_ux.md
**Baseline**: 364
**Audit total**: 3
...
```
Audit report (2026-07-12T09:49:13Z)
=============================================
  mock_predictions                  0
...
Gate: total (3) must be **<=** baseline (364). Strictly **<** unless stage is flagged `--no-baseline-drop`.
...
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE** (same class as
  Stage-24/25/26/27): the coarse heuristic matches the word "actuator" across the whole tree — `.pyc` bytecode,
  `backend/.pytest_cache/...` cache files, gitignored venv third-party (redis/paho-mqtt/aioredis), and the
  FIRST-COMMIT legacy demo agents (the G-082 surface). Stage 28 added NO actuator emitter — the GraphRAG retriever
  (`knowledge_graph/graphrag.py`) and the adoption endpoints (`api/adoption_routes.py`) are read-only/advisory and
  never actuate; the sole actuator emitter remains `master.dispatch_order` (validator-gated + trace-paired, enforced
  by the CI invariant `scripts/check-safety-trace-pairing.py`, not this grep). Regression 49 passed / 1 skipped / 0
  failed live this session; audit-chain verify exit 0.
- [x] Audit count 364 → 3. GENUINE STRICT DECREASE (no `--no-baseline-drop` needed) — G-085 venv-scoping hygiene
  (~209-212 gitignored third-party lines) PAIRED with the real G-082 de-mock (0 `random.*` in the project backend +
  0 `Math.random` in `frontend-nextjs/src`). Decomposed for the reviewer in ADR
  `2026-07-04_stage28_graphrag_adoption_ux.md`; independent review confirmed LEGITIMATE (not gaming); real project
  fabrication = 0.
