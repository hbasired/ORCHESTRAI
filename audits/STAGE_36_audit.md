# Audit report — Stage 36

**Date**: 2026-07-18T08:21:27Z
**Task doc**: tasks/STAGE_36_TBD.md
**Baseline**: 3
**Audit total**: 3
...
```
Audit report (2026-07-18T08:21:06Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 3. **WAIVED via `--no-baseline-drop`** — Stage 36 is a DOCS-ONLY dependency-refresh
  feasibility ASSESSMENT (`compliance/dependency-refresh-assessment.md` + ledger notes + explainer). NO
  requirements.txt / lockfile / code changed; the working env is verified UNCHANGED (all pins intact). No fakery
  pattern can be introduced by markdown; residual 3 = the documented `_generate_heuristic_actions` G-052 false-positive.
  Justified in `KB_TASK_LOG.md`; ADR `2026-07-18_stage36_dependency_refresh_assessment.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE + not-applicable** — the
  coarse heuristic matches "actuator" across pre-existing `.pyc`/venv/legacy backend files; Stage 36 touched NO code at
  all (docs-only). The sole actuator emitter remains `master.dispatch_order` (validator + trace-paired, now
  capability-token-guarded per Stage 33). Backend audit + chain unaffected.
