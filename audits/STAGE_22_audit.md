# Audit report — Stage 22

**Date**: 2026-06-26T15:05:43Z
**Task doc**: tasks/STAGE_22_TBD.md
**Baseline**: 364
**Audit total**: 364
...
```
Audit report (2026-06-26T15:05:34Z)
=============================================
  mock_predictions                  0
...
compliance/decision-logs/2026-06-22_stage22_pilot_deployment_runbook.md
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — Stage 22 is **additive governance docs + a DB migration + a test-isolation fixture + a CI job**: `compliance/*.md` runbooks/plans, migration `0009` (role grant), `mem0_adapter` direct-login (real defence, no theatre), `audit_chain` DSN seam, a conftest fixture, a new crypto-gate CI job. No `random.*`/mock/`RESPONSES={}`/`MODELS=[]` introduced; no existing theatre in scope to remove. Every claim measured live (R8 RLS by-role; R1 chain head 421 untouched; R6 17 crypto tests pass on OpenSSL 3.5). Justified in `knowledge-base/KB_TASK_LOG.md` (Stage 22 entry); ADR `2026-06-22_stage22_pilot_deployment_runbook.md`.
- [x] Actuator-touching code without safety.validator reference (Stage 17+). **FALSE POSITIVE (persistent, pre-existing).** Stage 22 added NO actuator path — the heuristic flags the same vendored `backend/venv/...` files it has flagged since Stage 19–21 (and the runbook §4 / risk-register text that *mentions* actuator/sil_bridge in describing the go-live wiring R5, which is documentation, not an emitter). The only real actuator emitter stays `master.dispatch_order`, validator-gated + trace-paired (Stage 17 CI `safety-contract-tests`).
