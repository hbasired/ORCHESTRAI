# Audit report — Stage 11

**Date**: 2026-06-15T04:57:26Z
**Task doc**: tasks/STAGE_11_langgraph_runtime.md
**Baseline**: 364
**Audit total**: 364
...
```
Audit report (2026-06-15T04:57:22Z)
=============================================
  mock_predictions                  0
...
- [x] Audit count flat at 364. WAIVED via `--no-baseline-drop` — justified in `knowledge-base/KB_TASK_LOG.md` (Stage 11 increments 4 & 5): the runtime de-mock (G-052) + the two infra fixes (G-053/G-054) are correctness/dict-literal changes the audit grep cannot see (Rule 1a audit-invisible theatre); the depth + runtime increments are additive (real models wired into a durable LangGraph). No grep-counted theatre exists in the touched files to remove. ADR `2026-06-15_stage11_full_infra_verification.md`.
